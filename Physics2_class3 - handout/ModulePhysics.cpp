...
bool ModulePhysics::Start()
{
	...
	world->SetContactListener(this); // <-- punter al mòdul physics
...
bool PhysBody::Contains(int x, int y) const
{
	...
	b2Fixture* test;
	b2Vec2* point = new b2Vec2(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));

	for (test = body->GetFixtureList(); test != NULL; test = test->GetNext())
	{
		if (test->TestPoint(*point))
		{
			return true;
		}
	}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	/*
	b2Fixture* test;
	b2Vec2* x0 = new b2Vec2(PIXEL_TO_METERS(x1), PIXEL_TO_METERS(y1));
	b2Vec2* x1 = new b2Vec2(PIXEL_TO_METERS(x2), PIXEL_TO_METERS(y2));
